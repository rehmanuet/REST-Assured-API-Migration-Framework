...
import com.cdk.qa.framework.utils.FlatMapUtil;
...
public class ApiValidationPOST extends ApiValidationBaseClass {
    ...
    private final String TEST_SUITE_POSTC = "postc";
    ...
    public void postEndpointValidation(String serviceName, String getUrl, String postUrl, String payload) {
        ...
        if ((originalResponse.jsonPath().get()) instanceof HashMap) {
            ...
            try {
                ...
                Map<String, Object> leftFlatMap = FlatMapUtil.flatten(Objects.requireNonNull(postRequestJson));
                Map<String, Object> rightFlatMap = FlatMapUtil.flatten(Objects.requireNonNull(originalJson));
...
//                ExtentTestManager.getTest().log(LogStatus.FAIL, "<span class='label failure'>: " + key + ": " + "Expected:" + value.leftValue() + " Actual: " + value.rightValue()));
                difference.entriesDiffering().forEach((key, value) -> ExtentTestManager.getTest().log(LogStatus.FAIL, "<span class='label failure'>: " + key + ": " + "Expected:" + value.leftValue() + " Actual: " + value.rightValue()));
    ...
    @Test(dataProvider = DATA_PROVIDER, groups = TEST_SUITE_POSTC, priority = 1)
    public void postcEndpointValidation(String serviceName, String getUrl, String postUrl, String payload) {
        ExtentTestManager.startTest(serviceName.concat(": POSTCRequestBody"), "postEndpointValidation");

        List<Map> originalResponseList;
        Map originalJson;

        postRequest(payload, postUrl);
        getSourceResponse(getUrl.concat(extracted));
